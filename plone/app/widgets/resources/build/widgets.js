...
/*jshint bitwise:true, curly:true, eqeqeq:true, immed:true, latedef:true,
  newcap:true, noarg:true, noempty:true, nonew:true, plusplus:true,
  undef:true, strict:true, trailing:true, browser:true, evil:true *//*global jQuery:false */(function(e,t,n){"use strict";var r=t.Base.extend({name:"plone-tabs",jqueryPlugin:"ploneTabs",defaults:{panels:".enableFormTabbing fieldset",panelKlass:"formPanel",tabKlass:"formTab",tabsKlass:"formTabs"},init:function(){var n=this;n.options=e.extend({},n.defaults,n.options),n.$panels=e(n.options.panels,n.$el);if(!n.$panels.size())return;n.$tabs=e("<ul/>").addClass(n.options.tabsKlass).insertBefore(n.$panels.parent()).attr({"data-pattern":"tabs"}),n.$panels.each(function(){var t=e(this).addClass(n.options.panelKlass),r=e("> legend",t).hide();n.$tabs.append(e("<li/>").addClass(n.options.tabKlass).append(e("<a/>").attr({id:r.attr("id"),href:"#"+t.attr("id")}).append(e("<span/>").html(r.text()))))}),t.initialize(n.$tabs)}});t.register(r)})(window.jQuery,window.Patterns);// tabs pattern.
//
// Author: Rok Garbas
// Contact: rok@garbas.si
// Version: 1.0
...
/*jshint bitwise:true, curly:true, eqeqeq:true, immed:true, latedef:true,
  newcap:true, noarg:true, noempty:true, nonew:true, plusplus:true,
  undef:true, strict:true, trailing:true, browser:true, evil:true *//*global jQuery:false */(function(e,t,n){"use strict";var r=t.Base.extend({name:"toggle",jqueryPlugin:"patternToggle",defaults:{attribute:"class",event:"click"},init:function(){var t=this;t.options=e.extend({},t.defaults,t.options),t.options.target?(t.$target=t.$el.closest(t.options.target),t.$target.size()===0&&(t.$target=t.closest(t.$el,t.options.target))):t.$target=t.$el,t.$target||e.error('No target found for "'+t.options.target+'".'),t.$el.on(t.options.event,function(n){n.stopPropagation(),(e.nodeName(n.target,"a")||e(n.target).parents("a").size()!==0)&&n.preventDefault(),t.toggle()})},closest:function(t,n){var r=this,i=e(n,t);if(i.size()===0){if(t.size()===0||e.nodeName(t[0],"body"))return;i=r.closest(t.parent(),n)}return i},isMarked:function(){var e=this;return e.options.attribute==="class"?this.$target.hasClass(this.options.value):this.$target.attr(this.options.attribute)===this.options.value},toggle:function(){var e=this;e.isMarked()?e.remove():e.add()},remove:function(){var e=this;e.$el.trigger("patterns.toggle.remove"),e.options.attribute==="class"?e.$target.removeClass(e.options.value):e.$target.removeAttr(e.options.attribute),e.$el.trigger("patterns.toggle.removed")},add:function(){var e=this;e.$el.trigger("patterns.toggle.add"),e.options.attribute==="class"?e.$target.addClass(e.options.value):e.$target.attr(e.options.attribute,e.options.value),e.$el.trigger("patterns.toggle.added")}});t.register(r)})(window.jQuery,window.Patterns);